...
func RunSetup(cfg *config.Config) error {
	...
	fmt.Println("     ./fafb --tui   (or ask Claude: setup_check)")
	fmt.Println()
...
func buildFields(cfg *config.Config) []wizardField {
	...
	return []wizardField{
		{key: "CLAUDE_DIR", label: "Claude data directory", description: "Where fafb stores sessions, secrets, sources, etc.", current: getEnvOr("CLAUDE_DIR", defaultClaudeDir), optional: true},
		...
		{key: "GREPTILE_REPO", label: "Greptile repo", description: "Default repo to query (e.g. github/owner/repo)", current: getEnvOr("GREPTILE_REPO", "github/caboose/fafb"), optional: true},
...
func writeEnvFile(path string, fields []wizardField) error {
	...
	sb.WriteString("# fafb configuration\n")
	sb.WriteString("# Generated by: fafb --setup\n\n")
